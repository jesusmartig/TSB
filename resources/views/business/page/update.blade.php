                ...
                <div class="d-sm-flex align-items-center justify-content-between border-bottom">
                    ...
                    <div>
                        <div class="btn-wrapper">
                            <a href="/dashboard/empresa-de-transporte" class="btn btn-primary text-white me-0">
                            ...
                            <div class="card-body">
                                ...
                                <div class="row pb-3">
                                    <div class="col-12 col-sm-2 col-md-2 col-lg-2 col-xl-2">
                                        ...
                                        <input type="text" class="form-control telephone" name="telephone" id="telephone" value="" autocomplete="off" required>
                                    </div>
                                    <div class="col-12 col-sm-2 col-md-2 col-lg-2 col-xl-2">
                                        <label class="col-form-label" for="phone">Celular</label>
                                        <input type="text" class="form-control phone" name="phone" id="phone" value="" autocomplete="off" required>
                                    </div>
                                    <div class="col-12 col-sm-4 col-md-4 col-lg-4 col-xl-4">
                                ...
                                <div class="row pb-3">
                                    ...
                                    <div class="col-12 col-sm-6 col-md-6 col-lg-6 col-xl-6">
                                        <label class="col-form-label" for="names">Nombre del representarle
                                            legal</label>
                                        <input type="text" class="form-control names" name="names" id="names" value="" autocomplete="off" required>
                                    </div>
...
@section('scripts')
    <script type="module">
        ...
        /* Obtenemos el valor de los campos a actualizar */
        requestService.Index(`/api/dashboard/transport-companies/show/${helpersService.getAllGetParams(3)}`).then(function (data) {
            $('.nit').val(data.attributes.nit);
            $('.digit').val(data.attributes.digit);
            $('.businessName').val(data.attributes.businessName);
            $('.typePerson').val(data.attributes.typePerson);
            $('.acronym').val(data.attributes.acronym);
            $('.nationalPersontype').val(data.attributes.nationalPersontype);
            $('.typesSociety').val(data.attributes.typesSociety);
            $('.economicSector').val(data.attributes.economicSector);
            $('.department').val(data.attributes.department);
            $('.city').val(data.attributes.city);
            $('.codeFuec').val(data.attributes.codeFuec);
            $('.telephone').val(data.attributes.telephone);
            $('.phone').val(data.attributes.phone);
            $('.direction').val(data.attributes.direction);
            $('.email').val(data.attributes.email);
            $('.typeDocuments').val(data.attributes.typeDocuments);
            $('.document').val(data.attributes.document);
            $('.names').val(data.attributes.names);

            requestService.Index(`/api/helpers/cities/show/cities/${data.attributes.city}`).then(function (data) {
                $.each(data, function (id, value) {
                    let html = `<option selected value="${value.id}">${value.attributes.cities}</option>`
                    $('.city').append(html)
                });
            })
        ...
        $.validator.setDefaults({
            submitHandler: function () {
                ...
                const url = `/api/dashboard/transport-companies/update/${helpersService.getAllGetParams(3)}`;
                requestService.Update(url, formData)
