@extends('layouts.app')
@section('title')
    Actualizar tarjeta de operación
@endsection
@section('content')
                            ...
                            <div class="row pb-3">
                                <div class="col-12 col-sm-5 col-md-5 col-lg-5 col-xl-5">
                                    ...
                                    <select class="custom-select form-control ocOperationcardtype"
                                        ...
                                        <option value="3">POR CONVENIO</option>
                                        <option value="4">POR CAMBIO DE EMPRESA</option>
                                    </select>
                                </div>
...
@section('scripts')
    <script type="module">
        ...
        OperationcardService.getShow(`/api/dashboard/operationcard/show/${Appservice.getAllGetParams(5)}`).then(function (operationcard) {
            $('#ocAffiliatecompany').val(operationcard.ocAffiliatecompany)
            $('#ocExpeditiondate').val(operationcard.ocExpeditiondate)
            $('#ocValiditystartdate').val(operationcard.ocValiditystartdate)
            $('#ocExpirationdate').val(operationcard.ocExpirationdate)
            $('#ocOperationcard').val(operationcard.ocOperationcard)
            $('#ocShares').val(operationcard.ocShares)
            $('#ocOperationcardtype').val(operationcard.ocOperationcardtype)
            $('#veId').val(operationcard.veId)
            $('#vcInternalnumber').val(operationcard.vcInternalnumber)
            $('#ocStatus').val(operationcard.ocStatus)

            $('.directory').val(`${operationcard.username}/vehiculos/${operationcard.veEnginenumber}/tarjeta_de_operacion`);
        ...
        $.validator.setDefaults({
            submitHandler: function () {
                ...
                const url = `/api/dashboard/operationcard/update/${Appservice.getAllGetParams(5)}`;
                OperationcardService.setUpdate(url, formData)
                Appservice.getMessage('Se ha actualizado correctamente', '/dashboard/vehiculos/documentos/tarjeta-de-operacion')
                return false;
            }
        })
